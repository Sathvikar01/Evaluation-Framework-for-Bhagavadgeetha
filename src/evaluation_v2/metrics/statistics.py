...
def mcnemar(a_hits: Iterable[bool], b_hits: Iterable[bool]) -> dict[str, float | int]:
    ...
    if len(a_hits) != len(b_hits): raise ValueError("McNemar requires aligned samples")
...
def compare_paired(a_rows: list[dict], b_rows: list[dict], *, seed: int = 20260803, repetitions: int = 2000) -> dict:
    ...
    for key in common:
        ...
        a_r1.append(bool(am.get("recall@1", 0)))
        b_r1.append(bool(bm.get("recall@1", 0)))
        a_mrr.append(float(am.get("mrr", 0) or 0)); b_mrr.append(float(bm.get("mrr", 0) or 0))
        a_ndcg.append(float(am.get("ndcg@10", 0) or 0)); b_ndcg.append(float(bm.get("ndcg@10", 0) or 0))
    return {"aligned_n": len(common), "a_only": sorted(set(a_by_id) - set(b_by_id)), "b_only": sorted(set(b_by_id) - set(a_by_id)),
            ...
            "changed_rankings": [key for key in common if a_by_id[key].get("retrieved_refs", []) != b_by_id[key].get("retrieved_refs", [])]}
